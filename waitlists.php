...
<?php
    ...
    if(isset($_POST["section_id"])){
        ...
        $stmt = $conn->prepare("DELETE FROM Waitlist WHERE student_id = ? AND section_id = ?;");
    ...
    if($_SESSION['role'] == 'student'){
        $stmt = "CALL GetWaitlistForStudentID($user_id);";
        $result = $conn->query($stmt);
    }
    else if($_SESSION['role'] == 'instructor'){
        $stmt = "CALL GetWaitlistForInstructorID($user_id);";
        $result = $conn->query($stmt);
    }
    else if($_SESSION['role'] == 'administrator'){
        $stmt = "CALL GetWaitlistForEveryone();";
        $result = $conn->query($stmt);
    }
?>
...
<body>
  ...
  <div class="wrapper-container">
    <div class="title-container">
      <?php echo "<h1>" . $_SESSION['first_name'] . " " . $_SESSION['last_name'] . "'s " . " Waitlist";?>
      <hr>
    </div>
    ...
    <table class="table table-hover" id="classTable">
      <thead class="table-light">
        <tr>
          ...
          <?php if ($_SESSION['role'] == 'instructor' || $_SESSION['role'] == 'administrator'): ?>
        ...
        <?php if ($result->num_rows > 0): ?>
          <?php while ($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td><form action="waitlists.php" method="post">
                  <button type="submit" class="btn btn-danger" <?php if($_SESSION['role'] == 'instructor' ){echo "disabled";}?>>Remove</button> <!-- Only students can add courses -->
                  <input type='hidden' name='section_id' value=<?php echo $row['section_id'];?>> <!-- sends the section_id in the POST request -->
                  <input type='hidden' name='student_id' value=<?php echo $row['student_id'];?>> <!-- sends the student_id in the POST request -->
                </form></td>
              </tr>
          <?php endwhile; ?>
        <?php else: ?>
